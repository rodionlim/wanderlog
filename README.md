...
- **Local trip photos** — browse device photos captured during the trip date range, switch between gallery and manage modes, filter by trip day or saved location metadata, run an in-app slideshow, preview photos full-screen, and delete Wandercraft references or request device-photo deletion from inside the app
